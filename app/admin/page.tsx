...
export default async function AdminDashboardPage() {
  ...
  const summaryCards: { label: string; value: number; status: DisplayStatus }[] = [
    { label: "Published jobs", value: counts.published, status: "published" },
    { label: "Draft jobs", value: counts.draft, status: "draft" },
    { label: "Scheduled jobs", value: counts.scheduled, status: "scheduled" },
    { label: "Closed / archived", value: counts.closedOrArchived, status: "closed" },
  ];
    ...
    <div>
      ...
      <div className="mb-6 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {summaryCards.map((card) => (
          <Link
            key={card.label}
            href={`/admin/jobs?status=${card.status}`}

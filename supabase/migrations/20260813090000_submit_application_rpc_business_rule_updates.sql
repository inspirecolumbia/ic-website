-- Application-form business-rule updates (2026-08-13 delegation batch):
-- 1. Phone is now required, not optional.
-- 2. College is now restricted to the fixed v_school_email_domains keys --
--    no more free-typed "Other" school. Keep in sync with lib/screening.ts's
--    SCHOOLS list.
...
-- free-typed "Other" bucket at the app layer (lib/screening.ts's
-- YEAR_OF_STUDY_OPTIONS), which can't be whitelisted the same way college
-- now is.
...
begin
  ...
  if jsonb_array_length(p_team_preferences) <> 3 then

-- anon has a direct EXECUTE grant on submit_application via the REST API,
-- so it can be called without ever going through the Next.js app -- meaning
-- lib/applications.ts's validation (email format, required fields, GPA
-- range, team/answer validity) can be bypassed entirely. That app-layer
-- validation is still correct and gives real users a friendly error before
-- this function is ever called, but it isn't the actual trust boundary.
-- This backstops the same checks here, at the boundary that matters.
--
-- Same signature as the original create function, so CREATE OR REPLACE
-- preserves the existing anon EXECUTE grant unchanged.
